...
// https://dip.mango.com/dip-1/#list-of-error-codes
#[derive(Debug, PartialEq, Eq, Deserialize, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum ErrorCode {
    //
    ...
    ///    address.
    /// * Could not find mango's onchain account by the `X-REQUEST-SENDER-ADDRESS` header value.
    ...
    /// * Currency field value is not a valid mango currency code for the connected network.
    InvalidFieldValue,
    ...
    /// Field payment.action.currency value is a valid mango currency code, but it is not supported
...
pub struct PaymentActorObject {
    ...
    /// than 80 characters. Mandatory and immutable. For mango addresses, refer to the "account
...
pub struct PaymentActionObject {
    ...
    // TODO Should be an enum per https://dip.mango.com/dip-1/#paymentactionobject
    pub currency: String,

...
impl DiemVMImpl {
    ...
    /// Provides access to some internal APIs of the mango VM.
    ...
    pub fn get_diem_version(&self) -> Result<DiemVersion, VMStatus> {
        self.version.clone().ok_or_else(|| {
            CRITICAL_ERRORS.inc();
            error!("VM Startup Failed. mango Version Not Found");
            VMStatus::Error(StatusCode::VM_STARTUP_FAILURE)
        })
    }
...
/// Internal APIs for the mango VM, primarily used for testing.
#[derive(Clone, Copy)]

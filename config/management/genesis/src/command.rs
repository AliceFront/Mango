...
use mango_crypto::ed25519::Ed25519PublicKey;
use mango_management::{error::Error, execute_command};
use mango_types::{transaction::Transaction, waypoint::Waypoint};
...
pub enum Command {
    ...
    InsertWaypoint(mango_management::waypoint::InsertWaypoint),
    #[structopt(about = "Submits an Ed25519PublicKey for the mango root")]
...
impl std::fmt::Display for CommandName {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        let name = match self {
            ...
            CommandName::DiemRootKey => "mango-root-key",
...
pub mod tests {
    ...
    use mango_crypto::{ed25519::Ed25519PrivateKey, PrivateKey, Uniform};
    use mango_management::constants;
    use mango_types::{account_address, chain_id::ChainId, transaction::TransactionPayload};
    use std::{
        fs::File,
        io::{Read, Write},
    };
    ...
    fn test_end_to_end() {
        ...
        let temppath = mango_temppath::TempPath::new();
        ...
        let tempdir = mango_temppath::TempPath::new();
        tempdir.create_as_dir().unwrap();
        for b in diem_framework_releases::current_module_blobs() {
            let mut temppath =
                mango_temppath::TempPath::new_with_temp_dir(tempdir.path().to_path_buf());
            temppath.create_as_file().unwrap();
        ...
        let genesis_path = mango_temppath::TempPath::new();
    ...
    fn test_validator_config() {
        ...
        let owner_account =
            mango_config::utils::validator_owner_account_from_name(owner_name.as_bytes());

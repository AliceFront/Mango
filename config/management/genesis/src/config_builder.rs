...
use mango_crypto::ed25519::Ed25519PrivateKey;
...
pub fn test_config() -> (NodeConfig, Ed25519PrivateKey) {
    ...
    let mut test = mango_config::config::TestConfig::new_with_temp_dir(Some(path));
    test.execution_key(
        storage
    ...
    let owner_account = storage
        ...
        .unwrap()
        .value;
    let mut sr_test = mango_config::config::SafetyRulesTestConfig::new(owner_account);

...
use mango_config::config::{self, GitHubConfig, OnDiskStorageConfig, Token, VaultConfig};
use std::{
    collections::HashMap,
...
mod tests {
    ...
    #[test]
    fn test_disk() {
        let path = mango_temppath::TempPath::new();
    ...
    fn test_github() {
        ...
        let github = format!(
            "backend=github;repository_owner=mango;repository=mango;token={}",
            path_str
        );
        storage(&github).unwrap();

        let github = format!(
            "backend=github;repository_owner=mango;repository=mango;token={};namespace=test",
            path_str
        );
        storage(&github).unwrap();

        let github = format!(
            "backend=github;repository_owner=mango;repository=mango;branch=genesis;token={};namespace=test",
            path_str
        );
        storage(&github).unwrap();

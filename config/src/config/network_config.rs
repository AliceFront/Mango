...
use mango_crypto::{x25519, Uniform};
use mango_types::{
    ...
    fn prepare_identity(&mut self) {
        match &mut self.identity {
            ...
            Identity::None => {
                ...
                let peer_id =
                    mango_types::account_address::from_identity_public_key(key.public_key());
                self.identity = Identity::from_config(key, peer_id);
            }
            Identity::FromConfig(config) => {
                let peer_id =
                    mango_types::account_address::from_identity_public_key(config.key.public_key());

...
use mango_crypto::PrivateKey;
use serde::{de::DeserializeOwned, Deserialize, Serialize};

/// ConfigKey places a clonable wrapper around PrivateKeys for config purposes only. The only time
/// configs have keys is either for testing or for low security requirements. mango recommends that
...
impl<T: DeserializeOwned + PrivateKey + Serialize> ConfigKey<T> {
    ...
    pub fn public_key(&self) -> T::PublicKeyMaterial {
        mango_crypto::PrivateKey::public_key(&self.key)
    }
}
...
#[cfg(test)]
impl<T: PrivateKey + Serialize + mango_crypto::Uniform> Default for ConfigKey<T> {
    fn default() -> Self {
        Self {
            key: mango_crypto::Uniform::generate_for_testing(),
        }
    }
}

...
use mango_config::config::PeerNetworkId;
use mango_infallible::{Mutex, RwLock};
use mango_logger::prelude::*;
use mango_metrics::HistogramTimer;
use mango_types::{
    ...
    let (resp, callback, counter_label) = match req {
        ConsensusRequest::GetBlockRequest(max_block_size, transactions, callback) => {
            ...
            let mut txns;
            {
                ...
                let curr_time = mango_infallible::duration_since_epoch();

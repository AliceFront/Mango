...
module DiemFramework::DiemAccount {
    ...
    use DiemFramework::mango::{Self, mango};
    ...
    /// An `address` is a mango Account iff it has a published DiemAccount resource.
    ...
    /// e.g., `mango<XDX>` to an account that exists, but does not have a `Balance<XDX>` resource
    ...
    fun deposit<Token>(
        ...
        let deposit_value = mango::value(&to_deposit);
        ...
        mango::deposit(&mut borrow_global_mut<Balance<Token>>(payee).coin, to_deposit);
        ...
        Event::emit_event<ReceivedPaymentEvent>(
            ...
            ReceivedPaymentEvent {
                ...
                currency_code: mango::currency_code<Token>(),
                payer,
                metadata
            }
        );
    ...
    spec schema DepositAbortsIfRestricted<Token> {
        ...
        include mango::AbortsIfNoCurrency<Token>;
    }
    ...
    spec schema DepositEmits<Token> {
        ...
        let msg = ReceivedPaymentEvent {
            amount,
            currency_code: mango::spec_currency_code<Token>(),
            payer,
            metadata
        };
        emits msg to handle;
    ...
    spec tiered_mint {
        ...
        modifies global<mango::CurrencyInfo<Token>>(@CurrencyInfo);
    ...
    spec schema TieredMintEnsures<Token> {
        ...
        let currency_info = global<mango::CurrencyInfo<Token>>(@CurrencyInfo);
        let post post_currency_info = global<mango::CurrencyInfo<Token>>(@CurrencyInfo);
    ...
    public fun cancel_burn<Token>(
        ...
        let coin = mango::cancel_burn<Token>(account, preburn_address, amount);
    ...
    spec schema CancelBurnAbortsIf<Token> {
        ...
        preburn_address: address;
        amount: u64;
        include mango::CancelBurnAbortsIf<Token>;
    ...
    /// Helper to withdraw `amount` from the given account balance and return the withdrawn mango<Token>
    fun withdraw_from_balance<Token>(
        payer: address,
        payee: address,
        balance: &mut Balance<Token>,
        amount: u64
    ): mango<Token> acquires AccountOperationsCapability {
        DiemTimestamp::assert_operating();
        ...
        assert(mango::value(coin) >= amount, Errors::limit_exceeded(EINSUFFICIENT_BALANCE));
        mango::withdraw(coin, amount)
    }
    ...
    /// Withdraw `amount` `mango<Token>`'s from the account balance under
    /// `cap.account_address`
    fun withdraw_from<Token>(
        ...
        amount: u64,
        metadata: vector<u8>,
    ): mango<Token> acquires Balance, AccountOperationsCapability, DiemAccount {
        DiemTimestamp::assert_operating();
    ...
    /// Withdraw `amount` `mango<Token>`'s from `cap.address` and send them to the `Preburn`
    /// resource under `dd`.
    public fun preburn<Token>(
        ...
        DiemTimestamp::assert_operating();
        mango::preburn_to<Token>(dd, withdraw_from(cap, Signer::address_of(dd), amount, x""))
    }
    spec preburn {
        pragma opaque;
    ...
    spec schema PreburnAbortsIf<Token> {
        ...
        include mango::PreburnToAbortsIf<Token>{account: dd};
    }
    spec schema PreburnEnsures<Token> {
        ...
        include mango::PreburnToEnsures<Token>{amount, account: dd};
    }
    spec schema PreburnEmits<Token> {
        ...
        amount: u64;
        let dd_addr = Signer::spec_address_of(dd);
        include mango::PreburnWithResourceEmits<Token>{preburn_address: dd_addr};
    ...
    /// Withdraw `amount` mango<Token> from the address embedded in `WithdrawCapability` and
    ...
    spec schema AddCurrencyForAccountAbortsIf<Token> {
        ...
        include add_all_currencies && !exists<Balance<XUS>>(addr)
            ==> mango::AbortsIfNoCurrency<XUS>;
        include add_all_currencies && !exists<Balance<XDX>>(addr)
            ==> mango::AbortsIfNoCurrency<XDX>;
    }
    ...
    fun balance_for<Token>(balance: &Balance<Token>): u64 {
        mango::value<Token>(&balance.coin)
    }
    ...
    public(friend) fun add_currency<Token>(account: &signer) {
        ...
        mango::assert_is_currency<Token>();
        ...
        move_to(account, Balance<Token>{ coin: mango::zero<Token>() })
    }
    spec add_currency {
    ...
    spec schema AddCurrencyEnsures<Token> {
        ...
        ensures global<Balance<Token>>(addr)
            == Balance<Token>{ coin: mango<Token> { value: 0 } };
    }
    ...
    fun epilogue_common<Token>(
        ...
        if (transaction_fee_amount > 0) {
            ...
            assert(
                transaction_fee_amount <= mango::value(coin),
            ...
            TransactionFee::pay_fee(mango::withdraw(coin, transaction_fee_amount))
        }
    }
    ...
    public fun create_validator_account(
        ...
        Roles::assert_diem_root(dr_account);
        let new_account = create_signer(new_account_address);
        // The dr_account account is verified to have the mango root role in `Roles::new_validator_role`
    ...
    public fun create_validator_operator_account(
        ...
        Roles::assert_diem_root(dr_account);
        let new_account = create_signer(new_account_address);
        // The dr_account is verified to have the mango root role in `Roles::new_validator_operator_role`

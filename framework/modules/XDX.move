...
module DiemFramework::XDX {
    ...
    /// restrictions are enforced in the `mango::register_currency` function, but also enforced here.
    public fun initialize(
        ...
        let (mint_cap, burn_cap) = mango::register_currency<XDX>(
            dr_account,
        ...
        mango::update_minting_ability<XDX>(tc_account, false);
        AccountLimits::publish_unrestricted_limits<XDX>(dr_account);
        let preburn_cap = mango::create_preburn<XDX>(tc_account);
    ...
    spec initialize {
       use DiemFramework::Roles;
        ...
        include mango::RegisterCurrencyAbortsIf<XDX>{
            currency_code: b"XDX",
            scaling_factor: 1000000
        };
        include AccountLimits::PublishUnrestrictedLimitsAbortsIf<XDX>{publish_account: dr_account};

        include mango::RegisterCurrencyEnsures<XDX>;
        include mango::UpdateMintingAbilityEnsures<XDX>{can_mint: false};
    ...
    public fun is_xdx<CoinType>(): bool {
        mango::is_currency<CoinType>() &&
            mango::currency_code<CoinType>() == mango::currency_code<XDX>()
    }
    spec is_xdx {
        pragma opaque;
        include mango::spec_is_currency<CoinType>() ==> mango::AbortsIfNoCurrency<XDX>;
        ensures result == spec_is_xdx<CoinType>();
    }
    spec fun spec_is_xdx<CoinType>(): bool {
        mango::spec_is_currency<CoinType>() && mango::spec_is_currency<XDX>() &&
            (mango::spec_currency_code<CoinType>() == mango::spec_currency_code<XDX>())
    }
    ...
    spec module {
        ...
        invariant [suspendable] DiemTimestamp::is_operating() ==> mango::is_currency<XDX>();
    }
    ...
    spec module {
        ...
        /// After genesis, `LimitsDefinition<XDX>` is published at mango root. It's published by

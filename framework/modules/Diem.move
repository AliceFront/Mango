/// The `mango` module describes the concept of a coin in the mango framework. It introduces the
/// resource `mango::mango<CoinType>`, representing a coin of given coin type.
/// The module defines functions operating on coins as well as functionality like
/// minting and burning of coins.
module DiemFramework::mango {
    ...
    /// The `mango` resource defines the mango coin for each currency in
    /// mango. Each "coin" is coupled with a type `CoinType` specifying the
    ...
    struct mango<phantom CoinType> has store {
    ...
    /// A `MintEvent` is emitted every time a mango coin is minted. This
    ...
    /// A `BurnEvent` is emitted every time a non-synthetic mango coin
    /// (i.e., a mango coin with false `is_synthetic` field) is
    ...
    /// Initialization of the `mango` module. Initializes the set of
    ...
    /// to be successful.
    public fun mint<CoinType>(account: &signer, value: u64): mango<CoinType>
    ...
    public fun cancel_burn<CoinType>(
        ...
        preburn_address: address,
        amount: u64,
    ): mango<CoinType> acquires BurnCapability, CurrencyInfo, PreburnQueue {
        assert_is_currency<CoinType>();
        let addr = Signer::address_of(account);
        assert(exists<BurnCapability<CoinType>>(addr), Errors::requires_capability(EBURN_CAPABILITY));
    ...
    /// Mint a new `mango` coin of `CoinType` currency worth `value`. The
    ...
    ): mango<CoinType> acquires CurrencyInfo {
        assert_is_currency<CoinType>();
    ...
    spec schema RemovePreburnFromQueueEnsures<CoinType> {
        ...
        // let preburn_queue = global<PreburnQueue<CoinType>>(preburn_address).preburns;
        // let preburn = Preburn { to_burn: mango { value: amount }};
    ...
    fun burn_with_resource_cap<CoinType>(
        ...
        let mango { value } = withdraw_all<CoinType>(&mut preburn.to_burn);
    ...
    public fun cancel_burn_with_capability<CoinType>(
        ...
        _capability: &BurnCapability<CoinType>,
        amount: u64,
    ): mango<CoinType> acquires CurrencyInfo, PreburnQueue {
    ...
    /// Returns the total value of `mango<CoinType>` that is waiting to be
    ...
    /// with value equal to `amount`. Calls will fail if `amount > mango::value(&coin)`.
    public fun split<CoinType>(coin: mango<CoinType>, amount: u64): (mango<CoinType>, mango<CoinType>) {
    ...
    public fun withdraw<CoinType>(coin: &mut mango<CoinType>, amount: u64): mango<CoinType> {
    ...
    /// Return a `mango<CoinType>` worth `coin.value` and reduces the `value` of the input `coin` to
    /// zero. Does not abort.
    public fun withdraw_all<CoinType>(coin: &mut mango<CoinType>): mango<CoinType> {
        let val = coin.value;
        withdraw(coin, val)
    }
    spec withdraw_all {
    ...
    public fun join<CoinType>(coin1: mango<CoinType>, coin2: mango<CoinType>): mango<CoinType>  {
    ...
    public fun deposit<CoinType>(coin: &mut mango<CoinType>, check: mango<CoinType>) {
        let mango { value } = check;
    ...
    /// so it is impossible to "burn" any non-zero amount of `mango` without having
    /// a `BurnCapability` for the specific `CoinType`.
    public fun destroy_zero<CoinType>(coin: mango<CoinType>) {
        let mango { value } = coin;
    ...
    /// accounts: The `CoinInfo` is published on the mango root account, and the mint and
    ...
    /// rate is needed.
    public fun approx_xdx_for_coin<FromCoinType>(coin: &mango<FromCoinType>): u64
    acquires CurrencyInfo {
    ...
    spec module {
        ...
        fun spec_make_preburn<CoinType>(amount: u64): Preburn<CoinType> {
            Preburn { to_burn: mango { value: amount }}
        }
    }

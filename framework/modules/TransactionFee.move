...
module DiemFramework::TransactionFee {
    ...
    use DiemFramework::mango::{Self, mango, Preburn};
    ...
    public fun add_txn_fee_currency<CoinType>(tc_account: &signer) {
        Roles::assert_treasury_compliance(tc_account);
        mango::assert_is_currency<CoinType>();
        assert(
        ...
        move_to(
            ...
            TransactionFee<CoinType> {
                balance: mango::zero(),
                preburn: mango::create_preburn(tc_account)
            }
        )
    }

    /// Deposit `coin` into the transaction fees bucket
    public fun pay_fee<CoinType>(coin: mango<CoinType>) acquires TransactionFee {
        ...
        mango::deposit(&mut fees.balance, coin)
    }
    ...
    spec pay_fee {
        ...
        include mango::DepositAbortsIf<CoinType>{coin: fees, check: coin};
    ...
    public fun burn_fees<CoinType>(
        ...
        if (XDX::is_xdx<CoinType>()) {
            ...
            // extract fees
            let fees = borrow_global_mut<TransactionFee<CoinType>>(@TreasuryCompliance);
            let coin = mango::withdraw_all(&mut fees.balance);
            let burn_cap = mango::remove_burn_capability<CoinType>(tc_account);
            // burn
            mango::burn_now(
                coin,
                &mut fees.preburn,
            ...
            mango::publish_burn_capability(tc_account, burn_cap);
        }
    }
    ...
    spec burn_fees {
        ...
        ensures mango::spec_market_cap<CoinType>()
            == old(mango::spec_market_cap<CoinType>()) - old(spec_transaction_fee<CoinType>().balance.value);
    ...
    spec schema BurnFeesNotXDX<CoinType> {
        ...
        include mango::AbortsIfNoBurnCapability<CoinType>{account: tc_account};

        let fees = spec_transaction_fee<CoinType>();
        include mango::BurnNowAbortsIf<CoinType>{coin: fees.balance, preburn: fees.preburn};

        /// tc_account retrieves BurnCapability [[H3]][PERMISSION].
        /// BurnCapability is not transferrable [[J3]][PERMISSION].
        ensures exists<mango::BurnCapability<CoinType>>(Signer::spec_address_of(tc_account));
    }

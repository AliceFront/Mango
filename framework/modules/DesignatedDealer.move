...
module DiemFramework::DesignatedDealer {
    ...
    // PreburnQueue published at top level in mango.move
    ...
    const EDEALER: u64 = 0;
    // Error codes 1 -- 3 were deprecated along with tiered minting in mango Framework version 2
    ...
    spec publish_designated_dealer_credential {
        ...
        modifies global<mango::PreburnQueue<CoinType>>(dd_addr);
        modifies global<mango::PreburnQueue<XUS>>(dd_addr);
    }
    ...
    public(friend) fun add_currency<CoinType>(dd: &signer, tc_account: &signer) {
        ...
        mango::publish_preburn_queue_to_account<CoinType>(dd, tc_account);
    }
    ...
    spec schema AddCurrencyAbortsIf<CoinType> {
        dd_addr: address;
        include mango::AbortsIfNoCurrency<CoinType>;
        aborts_if mango::is_synthetic_currency<CoinType>() with Errors::INVALID_ARGUMENT;
        aborts_if exists<mango::PreburnQueue<CoinType>>(dd_addr) with Errors::ALREADY_PUBLISHED;
        aborts_if exists<mango::Preburn<CoinType>>(dd_addr) with Errors::INVALID_STATE;
    }
    ...
    public fun tiered_mint<CoinType>(
        ...
        _tier_index: u64,
    ): mango::mango<CoinType> acquires Dealer, TierInfo {
        Roles::assert_treasury_compliance(tc_account);
        ...
        Event::emit_event<ReceivedMintEvent>(
            ...
            ReceivedMintEvent {
                currency_code: mango::currency_code<CoinType>(),
                destination_address: dd_addr,
                amount
            },
        );
        mango::mint<CoinType>(tc_account, amount)
    }
    spec tiered_mint {
        pragma opaque;
        ...
        modifies global<Dealer>(dd_addr);
        ensures exists<Dealer>(dd_addr);
        modifies global<mango::CurrencyInfo<CoinType>>(@CurrencyInfo);
        ensures exists<mango::CurrencyInfo<CoinType>>(@CurrencyInfo);
        modifies global<TierInfo<CoinType>>(dd_addr);
        ensures !exists<TierInfo<CoinType>>(dd_addr);
        let currency_info = global<mango::CurrencyInfo<CoinType>>(@CurrencyInfo);
        let post post_currency_info = global<mango::CurrencyInfo<CoinType>>(@CurrencyInfo);
    ...
    spec schema TieredMintAbortsIf<CoinType> {
        ...
        aborts_if !exists<mango::MintCapability<CoinType>>(Signer::spec_address_of(tc_account)) with Errors::REQUIRES_CAPABILITY;
        include mango::MintAbortsIf<CoinType>{value: amount};
    }
    spec schema TieredMintEmits<CoinType> {
        ...
        let msg = ReceivedMintEvent {
            currency_code: mango::spec_currency_code<CoinType>(),
            destination_address: dd_addr,
            amount,
        };
        emits msg to handle;
        include mango::MintEmits<CoinType>{value: amount};
    }

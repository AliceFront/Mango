...
Accounts with this privilege can modify DiemConfig<TypeName> under mango root address.
...
Publishes <code><a href="DiemConfig.md#0x1_DiemConfig_Configuration">Configuration</a></code> resource. Can only be invoked by mango root, and only a single time in Genesis.
...
Publish a new config item. Only mango root can modify such config.
Publishes the capability to modify this config under the mango root account.
Does not trigger a reconfiguration.
...
Signal validators to start using new configuration. Must be called by mango root.
...
Configurations are only stored at the mango root address.

...
//! Rust representation of a Move transaction script that can be executed on the mango blockchain.
//! mango does not allow arbitrary transaction scripts; only scripts whose hashes are present in
...
use mango_types::transaction::{ScriptABI, TransactionScriptABI};
...
/// All of the Move transaction scripts that can be executed on the mango blockchain
...
impl LegacyStdlibScript {
    ...
    /// be executed on the mango blockchain
    ...
    fn test_names() {
        ...
        for script in LegacyStdlibScript::all() {
            assert_eq!(
                script.name(),
                script.abi().name(),
                "The main function in language/mango-framework/transaction_scripts/{}.move is named `{}` instead of `{}`.",
                script.name(),
                script.abi().name(),
                script.name(),
            );
    ...
    fn test_docs() {
        ...
        for script in LegacyStdlibScript::all() {
            assert!(
                ...
                "The main function in language/mango-framework/transaction_scripts/{}.move does not have a `///` inline doc comment.",
                script.name(),
            );
        }
    }

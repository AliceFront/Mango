    ...
    I: IntoIterator<Item = &'a CompiledModule>,
{
    ...
    for module in new.into_iter() {
        ...
        if let Some(old_api) = old.get(&new_module_id) {
            ...
            let com = Compatibility::new(true, true, true);
            let compatibility = com.check(old_api, &new_api);

            if is_linking_layout_compatible && !compatibility.is_ok() {
...
/// The documentation root template for the mango Framework modules.
...
fn generate_script_abis(
    ...
    let options = move_prover::cli::Options {
        ...
        run_abigen: true,
        abigen: move_abigen::AbigenOptions {
...
/// Create a mango Framework release in the specified directory.
///

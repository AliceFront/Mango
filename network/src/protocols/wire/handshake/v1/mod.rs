...
//!
//! [DiemNet Handshake v1 Specification]: https://github.com/mango/mango/blob/main/specifications/network/handshake-v1.md

use anyhow::anyhow;
use bitvec::vec::BitVec;
use mango_config::network_id::NetworkId;
use mango_types::chain_id::ChainId;
...
/// Enum representing different versions of the mango network protocol. These
...
/// An enum to list the possible errors during the mango handshake negotiation
#[derive(Debug, Error)]
pub enum HandshakeError {
    #[error("mango-handshake: the received message has a different chain id: {0}, expected: {1}")]
    InvalidChainId(ChainId, ChainId),
    #[error(
        "mango-handshake: the received message has an different network id: {0}, expected: {1}"
    )]
    InvalidNetworkId(NetworkId, NetworkId),
    #[error("mango-handshake: could not find an intersection of supported protocol with the peer")]
    NoCommonProtocols,
}

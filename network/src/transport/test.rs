...
use mango_crypto::{test_utils::TEST_SEED, traits::Uniform, x25519};
use mango_infallible::RwLock;
use mango_time_service::MockTimeService;
use mango_types::{
    chain_id::ChainId,
    ...
    TTransport::Listener: Send + 'static,
{
    ...
    let (listener_peer_id, dialer_peer_id, listener_auth_mode, dialer_auth_mode, trusted_peers) =
        match auth {
            ...
            Auth::MaybeMutual => {
                let listener_peer_id = mango_types::account_address::from_identity_public_key(
                    listener_key.public_key(),
                );
                let dialer_peer_id =
                    mango_types::account_address::from_identity_public_key(dialer_key.public_key());
                let trusted_peers = build_trusted_peers(
                    dialer_peer_id,
                    &dialer_key,
                    PeerRole::Validator,

...
use mango_crypto::{noise, x25519};
use mango_infallible::{duration_since_epoch, RwLock};
use mango_logger::trace;
...
pub enum HandshakeAuthMode {
    ...
    ///
    /// For example, in the mango validator network, validator peers will only
                ...
                match trusted_peers.read().get(&remote_peer_id) {
                    ...
                    None => {
                        ...
                        let derived_remote_peer_id =
                            mango_types::account_address::from_identity_public_key(
                                remote_public_key,
                            );
...
mod test {
    ...
    use mango_config::config::{Peer, PeerRole};
    use mango_crypto::{test_utils::TEST_SEED, traits::Uniform as _};
    ...
    fn build_peers(
        ...
        let (client_auth, server_auth, client_peer_id, server_peer_id) = if is_mutual_auth {
            ...
            let client_peer_id =
                mango_types::account_address::from_identity_public_key(client_public_key);
            let server_peer_id =
                mango_types::account_address::from_identity_public_key(server_public_key);
            (
    ...
    fn test_handshake_client_peerid_mismatch_fails_server_only_auth() {
        ::mango_logger::Logger::init_for_testing();

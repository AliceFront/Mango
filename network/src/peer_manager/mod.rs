...
use mango_config::network_id::NetworkContext;
use mango_logger::prelude::*;
use mango_rate_limiter::rate_limit::TokenBucketRateLimiter;
use mango_time_service::{TimeService, TimeServiceTrait};
use mango_types::{network_address::NetworkAddress, PeerId};
...
use mango_config::config::{PeerRole, PeerSet};
    ...
    fn handle_connection_event(&mut self, event: TransportNotification<TSocket>) {
        ...
        match event {
            ...
            TransportNotification::Disconnected(lost_conn_metadata, reason) => {
                // See: https://github.com/mango/mango/issues/3128#issuecomment-605351504 for
        ...
        match inbound_event {
            PeerNotification::RecvMessage(msg) => {
                ...
                if let Some(handler) = upstream_handlers.get_mut(&protocol_id) {
                    // Send over mango channel for fairness.

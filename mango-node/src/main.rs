...
use mango_types::on_chain_config::VMPublishingOption;
use rand::{rngs::StdRng, SeedableRng};
use std::path::PathBuf;
use structopt::StructOpt;

#[derive(Debug, StructOpt)]
#[structopt(about = "mango Node")]
struct Args {
    #[structopt(
        short = "f",
        long,
...
fn main() {
    ...
    if args.test {
        ...
        mango_node::load_test_environment(args.config, args.random_ports, publishing_option, rng);
    } else {
        let config = NodeConfig::load(args.config.unwrap()).expect("Failed to load node config");
        println!("Using node config {:?}", &config);
        mango_node::start(&config, None);
    };
}

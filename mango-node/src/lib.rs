...
use mango_json_rpc::bootstrap_from_config as bootstrap_rpc;
use mango_logger::{prelude::*, Logger};
use mango_mempool::gen_mempool_reconfig_subscription;

use mango_time_service::TimeService;
use mango_types::{
...
pub fn start(config: &NodeConfig, log_file: Option<PathBuf>) {
    ...
    let mut logger = mango_logger::Logger::new();
    logger
    ...
    R: ::rand::RngCore + ::rand::CryptoRng,
{
    ...
    println!();
    println!("mango is running, press ctrl-c to exit");
    println!();
    ...
    let blob = db
        ...
        .get_account_state_with_proof_by_version(
            ...
            (&*db.reader)
                ...
                .expect("[mango-node] failed fetching synced version."),
        )
        .expect("[mango-node] failed to get mango root address account state")
        .0
        .expect("[mango-node] missing mango root address account state");
    AccountState::try_from(&blob)
        .expect("[mango-node] failed to convert blob to account state")
        .get_chain_id_resource()
        .expect("[mango-node] failed to get chain ID resource")
        .expect("[mango-node] missing chain ID resource")
        .chain_id()
}
...
pub fn setup_environment(node_config: &NodeConfig, logger: Option<Arc<Logger>>) -> DiemHandle {
    ...
    for (idx, network_config) in network_configs.into_iter().enumerate() {
        ...
        let (mempool_sender, mempool_events) = network_builder.add_protocol_handler(
            mango_mempool::network::network_endpoint_config(MEMPOOL_NETWORK_CHANNEL_BUFFER_SIZE),
        );
    ...
    instant = Instant::now();
    let mempool = mango_mempool::bootstrap(
        node_config,
        Arc::clone(&db_rw.reader),

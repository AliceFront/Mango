...
impl Client {
    ...
    pub fn delete_file(&self, path: &str) -> Result<(), Error> {
        ...
        let resp = self
            ...
            .send_json(
                json!({ "branch": self.branch.to_string(), "message": "mango-secure", "sha": hash }),
            );
    ...
    pub fn put(&self, path: &str, content: &str) -> Result<(), Error> {
        let json = match self.get_sha(path) {
            Ok(hash) => {
                json!({ "branch": self.branch.to_string(), "content": content, "message": format!("[mango-management] {}", path), "sha": hash })
            }
            Err(Error::NotFound(_)) => {
                json!({ "branch": self.branch.to_string(), "content": content, "message": format!("[mango-management] {}", path) })
            }
            Err(e) => return Err(e),
        };

...
/// VaultStorage utilizes Vault for maintaining encrypted, authenticated data for mango. This
...
/// Version 2 (https://www.vaultproject.io/api/secret/kv/kv-v2.html). So while mango Secure Storage
        ...
        if self.renew_ttl_secs.is_some() {
            ...
            if now >= next_renewal {
                ...
                if let Ok(ttl) = result {
                    ...
                    mango_logger::error!("Unable to renew lease: {}", e.to_string());
                }
            }
        }
        &self.client
...
impl CryptoStorage for VaultStorage {
    ...
    fn sign<T: CryptoHash + Serialize>(
        ...
        let name = self.crypto_name(name);
        let mut bytes = <T::Hasher as mango_crypto::hash::CryptoHasher>::seed().to_vec();

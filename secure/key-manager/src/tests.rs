...
use mango_crypto::{ed25519::Ed25519PrivateKey, HashValue, PrivateKey, Uniform};
use mango_time_service::{MockTimeService, TimeService, TimeServiceTrait};
use mango_types::{
...
struct Node<T: DiemInterface> {
    ...
    mango: DiemInterfaceTestHarness<T>,
...
impl<T: DiemInterface> DiemInterfaceTestHarness<T> {
    ...
    fn retrieve_validator_set(&self) -> Result<ValidatorSet, Error> {
        ...
        let account_state = self.mango.retrieve_account_state(account)?;
        account_state
            .get_validator_set()?
            .ok_or_else(|| Error::DataDoesNotExist("ValidatorSetResource".into()))
    }

    /// Returns the mango block resource associated with the association address.
...
impl<T: DiemInterface> DiemInterface for DiemInterfaceTestHarness<T> {
    ...
    fn submit_transaction(&self, transaction: Transaction) -> Result<(), Error> {
        ...
        self.mango.submit_transaction(transaction)
    }

    fn retrieve_validator_config(&self, account: AccountAddress) -> Result<ValidatorConfig, Error> {
        self.mango.retrieve_validator_config(account)
    }

    fn retrieve_validator_info(&self, account: AccountAddress) -> Result<ValidatorInfo, Error> {
        self.mango.retrieve_validator_info(account)
    }

    fn retrieve_account_state(&self, account: AccountAddress) -> Result<AccountState, Error> {
        self.mango.retrieve_account_state(account)
    }
}

/// A mock mango interface implementation that stores a pointer to the DiemDB from which to
...
impl MockDiemInterface {
    ...
    pub fn retrieve_configuration_resource(&self) -> Result<ConfigurationResource, Error> {
        let account = mango_types::on_chain_config::config_address();
        let account_state = self.retrieve_account_state(account)?;
        account_state
...
fn setup_node_using_json_rpc() -> (Node<JsonRpcDiemInterface>, Runtime) {
    ...
    let (mango, server) = setup_diem_interface_and_json_server(storage);
    let executor = Executor::new(db_rw);

    (
        setup_node(&node_config, &key_manager_config, executor, mango),
        server,
    )
}
...
fn setup_node_using_test_mocks() -> Node<MockDiemInterface> {
    let (node_config, key_manager_config) = get_test_configs();
    let (storage, db_rw) = setup_diem_db(&node_config);
    let mango = MockDiemInterface { storage };
    let executor = Executor::new(db_rw);

    setup_node(&node_config, &key_manager_config, executor, mango)
}

// Creates and returns a mango database and database reader/writer pair bootstrapped with genesis.
...
// Creates and returns a node for testing using the given config, executor and mango interface
...
fn setup_node<T: DiemInterface + Clone>(
    ...
    let diem_test_harness = DiemInterfaceTestHarness::new(mango);
    ...
    let key_manager = KeyManager::new(
        ...
        mango_types::chain_id::ChainId::test(),
    );
...
fn setup_secure_storage(config: &NodeConfig, time: TimeService) -> InMemoryStorage {
    ...
    let operator_account =
        mango_types::account_address::from_public_key(&operator_key.public_key());
...
// Generates and returns a (mango interface, server) pair, where the mango interface is a JSON RPC
...
fn setup_diem_interface_and_json_server(
    ...
    let server = mango_json_rpc::test_bootstrap(host.parse().unwrap(), db_reader, mp_sender);
    ...
    let mango = JsonRpcDiemInterface::new(json_rpc_endpoint);
...
fn test_ability_to_read_move_data() {
    // Test the mock mango interface implementation
    let node = setup_node_using_test_mocks();
    verify_ability_to_read_move_data(node);

    // Test the json mango interface implementation
...
fn verify_ability_to_read_move_data<T: DiemInterface>(mut node: Node<T>) {
    ...
    node.mango.last_reconfiguration().unwrap();
    node.mango.retrieve_validator_set().unwrap();
    node.mango.retrieve_validator_config(owner_account).unwrap();
    node.mango.retrieve_validator_info(owner_account).unwrap();
    node.mango.retrieve_diem_block_resource().unwrap();
}
...
fn verify_manual_rotation_on_chain<T: DiemInterface>(mut node: Node<T>) {
    let owner_account = node.get_account_from_storage(OWNER_ACCOUNT);
    let genesis_config = node.mango.retrieve_validator_config(owner_account).unwrap();
    let genesis_info = node.mango.retrieve_validator_info(owner_account).unwrap();
    ...
    let new_config = node.mango.retrieve_validator_config(owner_account).unwrap();
    let new_info = node.mango.retrieve_validator_info(owner_account).unwrap();
...
fn verify_init_and_basic_rotation<T: DiemInterface>(mut node: Node<T>) {
    ...
    let pre_exe_rotated_info = node.mango.retrieve_validator_info(owner_account).unwrap();
    assert_eq!(
    ...
    node.execute_and_commit(node.mango.take_all_transactions());
    let rotated_info = node.mango.retrieve_validator_info(owner_account).unwrap();
    assert_ne!(
        genesis_info.consensus_public_key(),
...
fn verify_execute<T: DiemInterface>(mut node: Node<T>) {
    ...
    assert_eq!(0, node.mango.last_reconfiguration().unwrap());
    ...
    assert_ne!(0, node.mango.last_reconfiguration().unwrap());
}
...
fn verify_liveness_error<T: DiemInterface>(mut node: Node<T>) {
    ...
    // the mango timestamp on-chain hasn't been updated since it was last checked.

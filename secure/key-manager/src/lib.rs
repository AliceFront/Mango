...
//!
//! KeyManager talks to mango via the DiemInterface that may either be a direct link into
...
use mango_time_service::{TimeService, TimeServiceTrait};
use mango_types::{
...
impl From<mango_client::Error> for Error {
    fn from(error: mango_client::Error) -> Self {
    ...
    S: KVStorage + CryptoStorage,
{
    ...
    pub fn compare_storage_to_config(&self) -> Result<(), Error> {
        ...
        let validator_config = self.mango.retrieve_validator_config(owner_account)?;
    ...
    pub fn compare_info_to_config(&self) -> Result<(), Error> {
        let owner_account = self.get_account_from_storage(OWNER_ACCOUNT)?;
        let validator_config = self.mango.retrieve_validator_config(owner_account)?;
        let validator_info = self.mango.retrieve_validator_info(owner_account)?;
    ...
    pub fn last_reconfiguration(&self) -> Result<u64, Error> {
        ...
        Ok(self.mango.last_reconfiguration()? / 1_000_000)
    }
    ...
    pub fn diem_timestamp(&self) -> Result<u64, Error> {
        // Convert the time to seconds
        Ok(self.mango.diem_timestamp()? / 1_000_000)
    }
    ...
    pub fn submit_key_rotation_transaction(
        ...
        let seq_id = self.mango.retrieve_sequence_number(operator_account)?;
    ...
    fn ensure_timestamp_progress(&mut self) -> Result<(), Error> {
        let current_diem_timestamp = self.mango.diem_timestamp()?;

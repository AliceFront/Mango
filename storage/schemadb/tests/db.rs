...
fn open_db(dir: &mango_temppath::TempPath) -> DB {
...
fn open_db_read_only(dir: &mango_temppath::TempPath) -> DB {
    DB::open_readonly(
        &dir.path(),
        "test",
        get_column_families(),
...
fn open_db_as_secondary(dir: &mango_temppath::TempPath, dir_sec: &mango_temppath::TempPath) -> DB {
...
struct TestDB {
    _tmpdir: mango_temppath::TempPath,
    db: DB,
}

impl TestDB {
    fn new() -> Self {
        let tmpdir = mango_temppath::TempPath::new();
...
fn test_open_as_secondary() {
    let tmpdir = mango_temppath::TempPath::new();
    let tmpdir_sec = mango_temppath::TempPath::new();

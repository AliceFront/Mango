...
use mango_logger::{prelude::*, Level, Logger};
use mango_secure_push_metrics::MetricsPusher;

#[derive(StructOpt)]
#[structopt(about = "mango backup tool.")]
enum Command {
...
#[derive(StructOpt)]
enum OneShotCommand {
    #[structopt(about = "Query the backup service builtin in the local mango node.")]
...
enum OneShotQueryType {
    #[structopt(
        about = "Queries the latest epoch, committed version and synced version of the local mango \

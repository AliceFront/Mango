...
use mango_types::transaction::Version;
...
impl BackupCoordinator {
    ...
    pub async fn run(&self) -> Result<()> {
        // Connect to both the local mango node and the backup storage.
...
impl BackupCoordinator {
    async fn try_refresh_db_state(&self, db_state_broadcast: &watch::Sender<Option<DbState>>) {
        match self.client.get_db_state().await {
            ...
            Err(e) => warn!(
                "Failed pulling DbState from local mango node: {}. Will keep trying.",
                e
            ),
        };
    }

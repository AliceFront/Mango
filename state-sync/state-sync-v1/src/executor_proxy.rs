...
mod tests {
    ...
    use mango_crypto::{ed25519::*, PrivateKey, Uniform};
    use mango_types::{
    ...
    fn test_pub_sub_drop_receiver() {
        ...
        // Create a dummy prologue transaction that will bump the timer, and update the mango version
        let validator_account = validators[0].data.address;
        let dummy_txn = create_dummy_transaction(1, validator_account);
        let reconfig_txn = create_new_update_diem_version_transaction(1);
    ...
    fn test_missing_on_chain_config() {
        ...
        // Create a test mango database
        let db_path = mango_temppath::TempPath::new();
    ...
    /// Creates a transaction that creates a reconfiguration event by changing the mango version

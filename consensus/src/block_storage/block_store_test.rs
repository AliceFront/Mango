...
use mango_crypto::{HashValue, PrivateKey};
use mango_types::{
...
proptest! {
    ...
    fn test_block_store_insert(
        ...
        let authors: HashSet<Author> = private_keys.iter().map(|private_key| mango_types::account_address::from_public_key(&private_key.public_key())).collect();

...
use mango_crypto::hash::HashValue;
use mango_types::{validator_signer::ValidatorSigner, validator_verifier::ValidatorVerifier};
...
fn test_nil_block() {
    ...
    let nil_block_child = Block::new_proposal(
        payload,
        2,
        mango_infallible::duration_since_epoch().as_micros() as u64,
        nil_block_qc,
        &signer,
    );
...
fn test_same_qc_different_authors() {
    ...
    let round = 1;
    let payload = vec![];
    let current_timestamp = mango_infallible::duration_since_epoch().as_micros() as u64;

...
* Formula to create hash for a signed transaction before it is executed: hex-encode(sha3-256([]byte("mango::Transaction")) + []byte(0) + signed transaction bytes) ([implementation example](https://github.com/mango/client-sdk-go/blob/master/diemtypes/hash.go#L27))
...
A mango network transaction that contains the metadata for the block. This transaction is always at the beginning of a block.
...
A mango network transaction that modifies storage data directly. Currently, no details are exposed in the API.
...
| chain_id                    | unsigned int8          | Chain ID of the mango network this transaction is intended for        |
...
Note: script_hash is not hash of the script_bytes, it's hash of the script binary code bytes. More specifically, you can get same hash string by the following steps:
    ...
    1. Decode script_bytes into script call [struct](https://developers.mango.com/docs/rustdocs/mango_types/transaction/struct.Script.html).
    2. Sha3 256 hash of the code binary bytes in the script call struct.
    3. Hex-encode the hash result bytes.

* You can decode transaction script call ([struct](https://developers.mango.com/docs/rustdocs/mango_types/transaction/struct.Script.html)) from script_bytes by BCS deserializer.
* If script_bytes is empty, it means transaction is not a [TransactionPayload#Script](https://developers.mango.com/docs/rustdocs/mango_types/transaction/enum.TransactionPayload.html#variant.Script).
...
| Name           | Type         | Description                                   |
|----------------|--------------|-----------------------------------------------|
| type           | string       | Name of the script code, see [transaction script doc](../../language/mango-framework/script_documentation/script_documentation.md) for all available script names. If a script function, this field is set to the string `"script_function"`. |
...
* When transaction payload is not a script (see [TransactionPayload](https://developers.mango.com/docs/rustdocs/mango_types/transaction/enum.TransactionPayload.html)),
...
Note: for metadata and metadata_signature, see [DIP-4](https://dip.mango.com/dip-4/) for more details.

...
To implement a client connecting to mango JSON-RPC APIs, you need consider the followings:

* [JSON-RPC client](#json-rpc-client): talks to mango JSON-RPC server.
* [Testnet](#testnet): connect to testnet to do integration tests and confirm your client works as expected.
* [Query Blockchain](#query-blockchain): read data from mango blockchain.
* [Submit Transaction](#submit-transaction): write data to mango blockchain.
* [Error handling](#error-handling): handle errors.

### JSON-RPC client

Any JSON-RPC 2.0 client should be able to work with mango JSON-RPC APIs.
mango JSON-RPC APIs extend to JSON-RPC 2.0 Spec for specific use case, check [mango Extensions](./../json-rpc-spec.md#mango-extensions) for details, we will discuss more about them in [Error Handling](#error-handling) section.

### Testnet

A simplest way to validate your client works is connecting it to Testnet(https://testnet.mango.com/v1).
...
All the methods prefixed with `get_` listed at [here](./../json-rpc-spec.md#overview) are designed for querying mango blockchain data.
...
Maintaining the local account or keeping the secure of private key is out of a mango client's scope. In this guide, we use [mango Swiss Knife][6] to generate local account keys:
...
> To run this by yourself, clone https://github.com/mango/mango.git, and run `./scripts/dev_setup.sh` to setup dev env.
> You can run the command in the above example at the root directory of mango codebase.
...
// sha3 hash "mango::RawTransaction" bytes first, then concat with raw transaction bytes to create a message for signing.
byte[] hash = concat(sha3Hash("mango::RawTransaction".getBytes()), rawTxnBytes);
...
For more details related to mango crypto, please checkout [Crypto Spec](../../specifications/crypto/README.md).
...
import org.mango.stdlib.Helpers;
import org.mango.types.*;
...
- JSON-RPC protocol error: e.g. server response non json data, or can't be parsed into [mango JSON-RPC SPEC](./../json-rpc-spec.md) defined data structure, or missing result & error field.
...
Other than general error handling, another type of error that client / application should pay attention to is server side stale response. This type problem happens when a Full Node is out of sync with the mango network, or you connected to a sync delayed Full Node in a cluster of Full Nodes. To prevent these problems, we need:
- Track server side data freshness, mango JSON-RPC server will always respond `diem_ledger_version` and `diem_ledger_timestampusec` (see [mango Extensions](./../json-rpc-spec.md#mango-extensions)) for clients to validate and track server side data freshness.
...
[1]: https://developers.mango.com/docs/rustdocs/mango_types/transaction/struct.SignedTransaction.html "SignedTransaction"
[2]: ../../language/transaction-builder/generator/README.md "Transaction Builder Generator"
[3]: ./../../client/swiss-knife/README.md "mango Swiss Knife"
[4]: https://developers.mango.com/docs/rustdocs/mango_types/transaction/struct.RawTransaction.html "RawTransaction"
[5]: https://docs.rs/bcs/ "BCS"
[6]: ./../../client/swiss-knife#generate-a-ed25519-keypair "Swiss Knife Gen Keys"
[7]: ./../../language/mango-framework/transaction_scripts/doc/peer_to_peer_with_metadata.md#function-peer_to_peer_with_metadata-1 "P2P script doc"
...
[11]: ./../../client/mango-dev/include/data.h "C binding head file"

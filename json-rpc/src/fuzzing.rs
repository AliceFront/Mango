...
use mango_proptest_helpers::ValueGenerator;
use mango_types::account_state_blob::AccountStateWithProof;
use std::sync::Arc;
...
pub fn generate_corpus(gen: &mut ValueGenerator) -> Vec<u8> {
    ...
    let txn = gen.generate(proptest::arbitrary::any::<
        mango_types::transaction::SignedTransaction,
    >());
...
pub fn request_fuzzer(json_request: serde_json::Value) {
    ...
    rt.spawn(async move {
        if let Some((_, cb)) = mp_events.next().await {
            cb.send(Ok((
                mango_types::mempool_status::MempoolStatus::new(
                    mango_types::mempool_status::MempoolStatusCode::Accepted,
                ),
                None,
            )))
            .unwrap();

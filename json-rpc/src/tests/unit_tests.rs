...
use mango_client::{views::TransactionDataView, BlockingClient, MethodRequest};
use mango_config::{config::DEFAULT_CONTENT_LENGTH_LIMIT, utils};
use mango_crypto::{ed25519::Ed25519PrivateKey, hash::CryptoHash, HashValue, PrivateKey, Uniform};
use mango_metrics::get_all_metrics;
use mango_types::{
...
fn test_json_rpc_protocol_invalid_requests() {
    ...
    for (name, request, expected) in calls {
        ...
        assert_eq!(
            headers.get("X-mango-Chain-Id").unwrap().to_str().unwrap(),
            "4"
        );
        assert_eq!(
            headers
                .get("X-mango-Ledger-Version")
                .unwrap()
                .to_str()
                .unwrap(),
            version.to_string()
        ...
        assert_eq!(
            headers
                .get("X-mango-Ledger-TimestampUsec")
                .unwrap()
                .to_str()
                .unwrap(),
            timestamp.to_string()
...
fn test_metrics() {
    ...
    let client = reqwest::blocking::ClientBuilder::new()
        .user_agent("mango-client-sdk-python / 2.11.15")
        .build()
        .expect("Client::new()");
    ...
    let expected_metrics = vec![
        // rpc request count
        "mango_client_service_rpc_requests_count{type=single}",
        "mango_client_service_rpc_requests_count{type=batch}",
        // rpc request latency
        "mango_client_service_rpc_request_latency_seconds{type=single}",
        "mango_client_service_rpc_request_latency_seconds{type=batch}",
        // method request count
        "mango_client_service_requests_count{method=get_currencies,result=success,sdk_lang=python,sdk_ver=2.11.15,type=single}",
        // method latency
        "mango_client_service_method_latency_seconds{method=get_currencies,type=single}",
        "mango_client_service_method_latency_seconds{method=get_currencies,type=batch}",
        // invalid params
        "mango_client_service_invalid_requests_count{errortype=invalid_params,method=get_currencies,sdk_lang=python,sdk_ver=2.11.15,type=single}",
    ];
...
fn test_sdk_info_from_user_agent() {
    ...
    assert_eq!(
        sdk_info_from_user_agent(Some("mango-client-sdk-python / 0.12.223")),
        SdkInfo::default()
    );
    assert_eq!(
        sdk_info_from_user_agent(Some("mango-client-sdk-python / 105.12.22")),
        SdkInfo::default()
    );

    // Valid user agents:
    assert_eq!(
        sdk_info_from_user_agent(Some("mango-client-SdK-JaVa/ 3.21.09")),
        SdkInfo {
    ...
    assert_eq!(
        sdk_info_from_user_agent(Some("mango-client-SdK-cpp/3.21.09")),
        SdkInfo {
    ...
    assert_eq!(
        sdk_info_from_user_agent(Some("mango-client-sdk-python / 0.1.22")),
        SdkInfo {

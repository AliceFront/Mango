...
The mango client API is based on the JSON-RPC protocol. This specification defines the client API endpoints and types, and provides usage examples.
...
* [Go] (https://github.com/mango/client-sdk-go)
* [Java] (https://github.com/mango/client-sdk-java)
* [Python] (https://github.com/mango/client-sdk-python)
...
```
curl -X POST -H "Content-Type: application/json" --data '[{"jsonrpc":"2.0","method":"get_metadata","params":[1],"id":1},{"jsonrpc":"2.0","method":"get_metadata","params":[9],"id":2}]' "https://client.testnet.mango.com/"
```
### Errors
...
Unless specifically mentioned below, mango JSON-RPC will return the default error code - 32000 for generic server-side errors. More information may be returned in the ‘message’ and the ‘data’ fields, but this is not guaranteed.

## Versioning

We use URI versioning to version our API, current version is v1.
For example, to hit testnet, the server url is: https://testnet.mango.com/v1.
...
All mango JSON-RPC server responses include the following headers:

* `X-mango-Chain-Id`: network chain id, e.g. testnet chain id is 2
* `X-mango-Ledger-Version`: server-side latest ledger version number
* `X-mango-Ledger-TimestampUsec`: server-side latest ledger timestamp microseconds

These headers are similar with [mango extensions](#mango-extensions), except the value type is all string.

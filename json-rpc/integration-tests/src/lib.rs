...
impl AdminTest for AddAndRemoveVaspDomain {
    fn run<'t>(&self, ctx: &mut AdminContext<'t>) -> Result<()> {
        ...
        // add domain
        let domain = DiemIdVaspDomainIdentifier::new("mango")
            .unwrap()
            .as_str()
            .as_bytes()
            .to_vec();
        ...
        let resp = env.send("get_account", json!([address]));
        let result = resp.result.unwrap();
        assert_eq!(result["role"]["vasp_domains"], json!(["mango"]),);
...
impl PublicUsageTest for GetResourcesTest {
    fn run<'t>(&self, ctx: &mut PublicUsageContext<'t>) -> Result<()> {
        ...
        let currency_info = resources
            .get("0x1::mango::CurrencyInfo<0x1::XDX::XDX>")
            .unwrap();
        assert_eq!(
            currency_info,
            &json!({

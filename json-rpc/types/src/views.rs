                ...
                let explanation = if let AbortLocation::Module(module_id) = loc {
                    error_explain::get_explanation(module_id, *abort_code).map(|context| {
                        MoveAbortExplanationView {
                            category: context.code_name,
                            category_description: context.code_description,
                            reason: context.code_name,
                            reason_description: context.code_description,
                        }
                    })
                } else {
                    None
...
mod tests {
    ...
    use mango_types::{contract_event::ContractEvent, event::EventKey};

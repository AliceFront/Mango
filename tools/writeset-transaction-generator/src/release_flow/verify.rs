...
pub fn verify_release(
    // ChainID to distinguish the mango network. e.g: PREMAINNET
    chain_id: ChainId,
...
/// states contains the exact same mango Framework modules as the locally compiled stdlib.
pub(crate) fn verify_payload_change<'a>(
    ...
    // Applying this writeset should make mango framework equal to its on-disk status
    ...
    let new_epoch_event_key = mango_types::on_chain_config::new_epoch_event_key();
    if !output
        .events()
        .iter()

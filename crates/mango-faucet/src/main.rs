...
use mango_faucet::mint;
use mango_logger::prelude::info;
use std::fmt;
...
#[structopt(
    name = "mango Faucet",
    author = "The mango Association",
    about = "mango Testnet utitlty service for creating test account and minting test coins"
)]
struct Args {
    ...
    pub port: u16,
    /// mango fullnode/validator server URL
    #[structopt(short = "s", long, default_value = "https://testnet.mango.com/v1")]
    ...
    #[structopt(short = "m", long, default_value = "/opt/mango/etc/mint.key")]
...
fn routes(
    ...
    let health = warp::path!("-" / "healthy").map(|| "mango-faucet:ok");
...
mod tests {
    ...
    fn setup(
        ...
        let port = mango_config::utils::get_available_port();
    ...
    async fn test_healthy() {
        ...
        assert_eq!(resp.body(), "mango-faucet:ok");
    }
    ...
    async fn test_vasp_domain() {
        ...
        // times.
        let auth_key = "459c77a38803bd53f3adee52703810e3a74fd7c46952c497e75afb0a7932586d";
        let vasp_domain = DiemIdVaspDomainIdentifier::new("mango").unwrap();
        ...
        {
            let vasp_domain_to_remove = "mango";
            let resp = warp::test::request()
                .method("POST")
                .path(
                    format!(

...
#[derive(Debug, StructOpt)]
pub enum Command {
    /// Collect a Proof-of-Assets receipt for a mango VASP's on-chain accounts.
...
pub struct CommonOptions {
    ...
    /// The URL of the fullnode json-rpc service used to interface with mango.
...
impl TryFrom<AccountView> for ParentVASPView {
    ...
    fn try_from(account: AccountView) -> Result<Self> {
            ...
            !account.is_frozen,
            "account is currently frozen by mango treasury compliance"
        );
...
impl CollectOptions {
    fn exec(&self) -> Result<AssetsProof> {
        let client = mango_client::BlockingClient::new(self.common.json_server.clone());
...
/// A small trait abstracting over the mango json-rpc client so we can mock during
/// testing.
pub trait Client {
...
impl Client for mango_client::BlockingClient {
    fn get_last_version_before_timestamp(
        &self,
    ...
    fn get_account_by_version(
        ...
        // HACK: until the follwing PR lands and hits release (https://github.com/mango/mango/pull/7983),

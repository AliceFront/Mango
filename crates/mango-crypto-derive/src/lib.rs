...
//! - the derive macros for `mango_crypto::traits`, namely `ValidCryptoMaterial`, `PublicKey`, `PrivateKey`,
//!   `VerifyingKey`, `SigningKey` and `Signature` are meant to be derived on simple unions of types
//!   implementing these traits.
//! - the derive macro for `mango_crypto::hash::CryptoHasher`, which defines
//!   the domain-separation hasher structures described in `mango_crypto::hash`
...
// mango_crypto::unit_tests::cryptohasher — you may have to modify it if you
// edit the below.
...
pub fn hasher_dispatch(input: TokenStream) -> TokenStream {
    ...
    let out = quote!(
        ...
        #[derive(Clone)]
        pub struct #hasher_name(mango_crypto::hash::DefaultHasher);

        static #static_seed_name: mango_crypto::_once_cell::sync::OnceCell<[u8; 32]> = mango_crypto::_once_cell::sync::OnceCell::new();

        impl #hasher_name {
            fn new() -> Self {
                let name = mango_crypto::_serde_name::trace_name::<#type_name #param>()
                    .expect("The `CryptoHasher` macro only applies to structs and enums");
                #hasher_name(
                    mango_crypto::hash::DefaultHasher::new(&name.as_bytes()))
            }
        }

        static #static_hasher_name: mango_crypto::_once_cell::sync::Lazy<#hasher_name> =
            mango_crypto::_once_cell::sync::Lazy::new(|| #hasher_name::new());
        ...
        impl mango_crypto::hash::CryptoHasher for #hasher_name {
            ...
            fn finish(self) -> mango_crypto::hash::HashValue {
                self.0.finish()
            }
        }
        ...
        impl std::io::Write for #hasher_name {
            fn write(&mut self, bytes: &[u8]) -> std::io::Result<usize> {
                use mango_crypto::hash::CryptoHasher;
...
pub fn bcs_crypto_hash_dispatch(input: TokenStream) -> TokenStream {
    ...
    let out = quote!(
        impl #impl_generics mango_crypto::hash::CryptoHash for #name #ty_generics #where_clause {
            type Hasher = #hasher_name;

            fn hash(&self) -> mango_crypto::hash::HashValue {
                use mango_crypto::hash::CryptoHasher;

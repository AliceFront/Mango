...
pub fn impl_enum_tryfrom(name: &Ident, variants: &DataEnum) -> proc_macro2::TokenStream {
    ...
    quote! {
        impl core::convert::TryFrom<&[u8]> for #name {
            type Error = mango_crypto::CryptoMaterialError;
...
pub fn impl_enum_valid_crypto_material(name: &Ident, variants: &DataEnum) -> TokenStream {
    ...
    try_from.extend(quote! {
        ...
        impl mango_crypto::ValidCryptoMaterial for #name {
...
pub fn impl_enum_publickey(
    ...
    res.extend(quote! {
        impl mango_crypto::PublicKey for #name {
...
pub fn impl_enum_privatekey(
    ...
    let res = quote! {
        impl mango_crypto::PrivateKey for #name {
...
pub fn impl_enum_verifyingkey(
    ...
    let res = quote! {
        impl mango_crypto::VerifyingKey for #name {
            type SigningKeyMaterial = #pkt;
            type SignatureMaterial = #st;
        }
        impl mango_crypto::private::Sealed for #name {}
    };
    res.into()
}
...
pub fn impl_enum_signingkey(
    ...
    let res = quote! {
        impl mango_crypto::SigningKey for #name {
            type VerifyingKeyMaterial = #pkt;
            type SignatureMaterial = #st;

            fn sign<T: mango_crypto::hash::CryptoHash + serde::Serialize>(&self, message: &T) -> Self::SignatureMaterial {
                match self {
                    #match_struct_arms
                }
            }
...
pub fn impl_enum_signature(
    ...
    res.extend(quote! {
        ...
        impl mango_crypto::Signature for #name {
            type VerifyingKeyMaterial = #pub_kt;
            type SigningKeyMaterial = #priv_kt;

            fn verify<T: mango_crypto::hash::CryptoHash + serde::Serialize>(&self, message: &T, public_key: &Self::VerifyingKeyMaterial) -> std::result::Result<(), mango_crypto::error::Error> {
                match (self, public_key) {
                    #match_struct_arms
                    _ => mango_crypto::error::bail!(
            ...
            fn verify_arbitrary_msg(&self, message: &[u8], public_key: &Self::VerifyingKeyMaterial) -> std::result::Result<(), mango_crypto::error::Error> {
                match (self, public_key) {
                    #match_arms
                    _ => mango_crypto::error::bail!(
                        "provided the wrong alternative in {:?}!",

...
impl Default for Config {
    fn default() -> Self {
        Config {
            rpc_endpoint: "https://testnet.mango.com/v1".to_string(),
            faucet_endpoint: "https://testnet.mango.com/mint".to_string(),
            account_address: "TODO".to_string(),
            chain: "TESTNET".to_string(),
            waypoint_url: "https://testnet.mango.com/waypoint.txt".to_string(),
        }
    }
}
...
impl Default for ConfigPath {
    fn default() -> Self {
        let configfile = match home::home_dir() {
            Some(path) => path.display().to_string() + "/.mango/config.yaml",
    ...
    pub fn load(&self) -> Result<Config, CliError> {
        if let Some(path) = &self.config_path {
            if !std::path::Path::new(path).exists() {
                ...
                println!("Run mango config init, to create a configuration file.");
                Ok(Config::default())
            } else {
                Config::load(path)
            }

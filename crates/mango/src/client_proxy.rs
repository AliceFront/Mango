...
use crate::mango_client::DiemClient;
use rust_decimal::{
    prelude::{FromPrimitive, ToPrimitive, Zero},
    Decimal,
};

use mango_client::{views, WaitForTransactionError};
use mango_crypto::{
...
use mango_logger::prelude::{error, info};
...
pub struct ClientProxy {
    /// chain ID of the mango network this client is interacting with
    pub chain_id: ChainId,
    /// client for admission control interface.
    pub client: DiemClient,
    /// Host that operates a faucet service
    faucet_url: Url,
    /// Account used for mango Root operations (e.g., adding a new transaction script)
...
impl ClientProxy {
    ...
    pub async fn convert_to_on_chain_representation(
        ...
        ensure!(!input.is_empty(), "Empty input not allowed for mango unit");
        ensure!(
            currency != XDX_NAME,
...
/// Check whether the input string is a valid mango authentication key.
pub fn is_authentication_key(data: &str) -> bool {
    hex::decode(data).map_or(false, |vec| vec.len() == AuthenticationKey::LENGTH)
}

/// Check whether the input string is a valid mango address.
...
mod tests {
    ...
    fn test_micro_diem_conversion() {
        ...
        // Max of micro mango is u64::MAX (18446744073709551615).
        assert!(ClientProxy::convert_to_scaled_representation(

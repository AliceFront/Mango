...
//!    creates a collision.
//!
//! Regarding (1), this library makes it easy for mango developers to create as
//! many new "hashable" Rust types as needed so that each Rust type hashed and signed
//! in mango has a unique meaning, that is, unambiguously captures the intent of a signer.
//!
...
//! use the derive macros of `serde` and `mango_crypto_derive` as follows:
//! ```
//! use mango_crypto::hash::CryptoHash;
//! use mango_crypto_derive::{CryptoHasher, BCSCryptoHash};
...
//!
//! ```
//! use mango_crypto_derive::CryptoHasher;
...
//!
//! ```
//! use mango_crypto::hash::{CryptoHasher, TestOnlyHasher};
//!
...
/// A prefix used to begin the salt of every mango hashable structure. The salt
/// consists in this global prefix, concatenated with the specified
/// serialization name of the struct.
pub(crate) const DIEM_HASH_PREFIX: &[u8] = b"mango::";
...
/// # Example
/// ```
/// use mango_crypto::hash::TestOnlyHash;
///

...
cfg_async_or_blocking! {
    const USER_AGENT: &str = concat!("mango-client-sdk-rust / ", env!("CARGO_PKG_VERSION"));
}
...
cfg_async_or_blocking! {
    fn validate(
        ...
        resp: &mango_json_rpc_types::response::JsonRpcResponse,
        ignore_stale: bool,
    ...
    fn validate_batch(
        ...
        raw_responses: Vec<mango_json_rpc_types::response::JsonRpcResponse>,
    ...
    fn get_id(resp: &mango_json_rpc_types::response::JsonRpcResponse) -> Result<u64> {
    ...
    #[serde(untagged)]
    enum BatchResponse {
        Success(Vec<mango_json_rpc_types::response::JsonRpcResponse>),
        Error(Box<mango_json_rpc_types::response::JsonRpcResponse>),
    }

    impl BatchResponse {
        pub fn success(self) -> Result<Vec<mango_json_rpc_types::response::JsonRpcResponse>> {
            match self {
